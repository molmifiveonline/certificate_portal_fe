...
const CreateCertificate = () => {
    ...
    setFormData((prev) => {
      ...
      if (name === "active_course_id") {
        ...
        if (selectedActive) {
          ...
          newData.course_conduct =
            selectedActive.type_of_location === "Online" ? "ONL" : "ONS";
          newData.trainer_id =
            selectedActive.primary_trainer_id || prev.trainer_id;
          if (!newData.course_id)
            newData.course_id = selectedActive.master_course_id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to create certificates",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-full h-full pb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-3 min-h-11 flex items-center">
                      {formData.candidate_ids.length > 0 ? (
                        ...
                        <span className="text-sm text-slate-500 italic">
                          No candidates selected for batch creation
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className={`w-full h-11 px-4 bg-slate-50 border ${formErrors.type ? "border-red-500" : "border-slate-200"} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm`}
                  >
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700">
                    Topic/Task <span className="text-red-500">*</span>
                  </label>
                  <select
                    name="topic"
                  ...
                  >
                    ...
                    {[
                      ...new Set(
                        masterCourses.map((c) => c.topic).filter(Boolean),
                      ),
                    ].map((topic, idx) => (
      ...
      <CandidateSelectionModal
        ...
        onSelectionChange={(ids) =>
          setFormData((prev) => ({ ...prev, candidate_ids: ids }))
        }
      />
    </div>
  );
