...
const EditCertificate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [candRes, mcRes, acRes, trRes, locRes, certRes] =
          await Promise.all([
        ...
        if (certRes) {
          setFormData({
            ...
            show_logo:
              certRes.show_logo === 1 && supportsStampLogo(certRes.type),
            ...
            <div className="space-y-6 grid grid-cols-1 gap-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  {console.log(candidates, "candidates777")}
                  <select
                    name="candidate_id"
                  ...
                  >
                    ...
                    {candidates.map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.first_name} {c.last_name} ({c.employee_id || "N/A"})
                      </option>
                    ))}
                  </select>
                  <p className="text-xs text-slate-500">
                    This screen updates one certificate record, so candidate
                    assignment stays single here. Multi-select is used on manual
