...
const AssessmentPlayer = ({ assessmentId, courseId, onClose }) => {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await api.get(
          `/assessment/${assessmentId}/play-questions`,
        );
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(answers).length < questions.length) {
      toast.warning(
        `Please answer all questions. You have answered ${Object.keys(answers).length} of ${questions.length}.`,
      );
      return;
    }
  ...
  if (result) {
    ...
    return (
      <div className="max-w-2xl mx-auto py-12 animate-in zoom-in-95 duration-300">
        <Card
          className={`overflow-hidden border-2 ${isPassed ? "border-emerald-100" : "border-rose-100"}`}
        >
          <div
            className={`h-2 w-full ${isPassed ? "bg-emerald-500" : "bg-rose-500"}`}
          />
          <CardHeader className="text-center pb-2">
            <div
              className={`mx-auto p-4 rounded-full w-fit mb-4 ${isPassed ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}
            >
              {isPassed ? (
          ...
          <CardContent className="space-y-8 py-8">
            <div className="flex justify-center gap-12">
              <div className="text-center">
                ...
                <div
                  className={`text-5xl font-black ${isPassed ? "text-emerald-500" : "text-rose-500"}`}
                >
                  {result.score}%
                </div>
              </div>
          ...
          <CardFooter className="bg-slate-50/50 p-6 flex justify-center">
            <Button
              className="px-10 h-12 text-lg font-bold rounded-xl"
              onClick={onClose}
            >
              Close & Continue
            </Button>
      ...
      <div className="flex items-center justify-between bg-white/80 backdrop-blur-xl p-4 rounded-2xl border border-white/60 shadow-sm">
        ...
        <div className="flex items-center gap-6">
          ...
          <Button
            ...
            size="sm"
            onClick={onClose}
            className="rounded-lg text-slate-400 hover:text-rose-500"
          >
            Exit
          </Button>
        </div>
      ...
      <Card className="bg-white/80 backdrop-blur-xl border-white/60 shadow-xl rounded-3xl overflow-hidden min-h-[400px] flex flex-col">
        <CardContent className="p-8 md:p-12 flex-grow">
          ...
          {currentQuestion.image && (
            <div className="mb-8">
              <img
                ...
                alt="Question"
                className="max-h-64 object-contain rounded-2xl border border-slate-200"
              />
            </div>
          )}
          ...
          <div className="space-y-4">
            {[
              { key: "option_a", imgKey: "opt_img_a" },
              { key: "option_b", imgKey: "opt_img_b" },
              { key: "option_c", imgKey: "opt_img_c" },
              { key: "option_d", imgKey: "opt_img_d" },
            ].map(({ key: optKey, imgKey }, idx) => {
              const optValue = currentQuestion[optKey];
              const optImage = currentQuestion[imgKey];
              if (!optValue && !optImage) return null;
                ...
                <div key={idx} className="relative group">
                  ...
                  >
                    <RadioGroupItem
                      ...
                      onChange={() =>
                        handleOptionSelect(currentQuestion.id, optKey)
                      }
                      name={`question-${currentQuestion.id}`}
                      className="mr-4 border-slate-300 text-primary"
                    />
                    <span
                      className={`text-base font-medium ${isSelected ? "text-primary" : "text-slate-700"} flex items-center gap-3`}
                    >
                      {optImage && (
                        <img
                          src={`${process.env.REACT_APP_API_URL || ""}/${optImage}`}
                          alt={`Option ${idx + 1}`}
                          className="max-h-20 object-contain rounded-lg border border-slate-200 mr-2"
                        />
                      )}
                      {optValue && <span>{optValue}</span>}
                    </span>
                  </Label>
                </div>
              );
