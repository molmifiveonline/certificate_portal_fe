  ...
  const fetchAssessments = async () => {
    try {
      const response = await api.get(
        `/assessment/course/${courseId}/candidate-list`,
      );
        ...
        {assessments.length === 0 ? (
          ...
          assessments.map((assessment) => {
            ...
            return (
              <Card
                key={assessment.id}
                className="bg-white/80 backdrop-blur-xl border-white/60 shadow-sm overflow-hidden group"
              >
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center text-sm text-slate-500">
                    ...
                    <span>
                      Latest Attempt:{" "}
                      {assessment.latest_attempt_date
                        ? formatDate(assessment.latest_attempt_date)
                        : "N/A"}
                    </span>
                  </div>
                </CardContent>
                <CardFooter className="bg-slate-50/50 border-t border-slate-100 flex flex-col gap-2 p-4">
                  {isPassed ? (
                  ...
                  {hasAttempted && assessment.latest_result_id && (
                    <Button
                      variant="outline"
                      className="w-full border-slate-200 hover:bg-slate-50 text-slate-700 font-medium"
                      onClick={() =>
                        setReviewResultId(assessment.latest_result_id)
                      }
                    >
