...
const HotelReport = () => {
  ...
  useEffect(() => {
    updateDebouncedHotel(searchTerm);
  }, [searchTerm, updateDebouncedHotel]);
  useEffect(() => {
    updateDebouncedEmployee(filterEmployee);
  }, [filterEmployee, updateDebouncedEmployee]);
  useEffect(() => {
    updateDebouncedCourse(filterCourse);
  }, [filterCourse, updateDebouncedCourse]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Course Name",
        render: (val) => (
          <span className="text-blue-700 font-medium">{val || "-"}</span>
        ),
      },
      {
        key: "course_dates",
