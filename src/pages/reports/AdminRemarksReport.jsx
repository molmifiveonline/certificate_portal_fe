...
const AdminRemarksReport = () => {
  ...
  const fetchReports = useCallback(async () => {
    try {
      setLoading(true);
      const data =
        await preActiveCourseService.getAdminRemarksReport(appliedFilters);
      setReports(data || []);
      setCurrentPage(1);
    } catch (error) {
      toast.error(
        getErrorMessage(error, "Failed to fetch admin remarks report"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const exportToPDF = () => {
    ...
    doc.save(
      `Admin_Remarks_Report_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      workbook,
      `Admin_Remarks_Report_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8">
      <Meta
        ...
        description="View and export admin remarks for candidate nominations"
      />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${
                                r.candidate_approval_status === "Approved"
                                  ? "bg-green-100 text-green-700 border border-green-200"
                                  : r.candidate_approval_status === "Rejected"
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${
                                r.admin_approval_status === "Approved"
                                  ? "bg-blue-100 text-blue-700 border border-blue-200"
                                  : r.admin_approval_status === "Rejected"
                                    ? "bg-red-100 text-red-700 border border-red-200"
