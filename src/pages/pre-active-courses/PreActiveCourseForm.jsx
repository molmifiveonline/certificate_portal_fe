...
const PreActiveCourseForm = () => {
    ...
    const validate = () => {
        ...
        if (!formData.type_of_course) newErrors.type_of_course = 'Course Type is required';
        ...
        if (Object.keys(newErrors).length > 0 && containerRef.current) {
            ...
            const errorElement = containerRef.current.querySelector(`[name="${firstErrorKey}"]`);
            if (errorElement) {
    ...
    return (
        <div className="min-h-screen bg-slate-50" ref={containerRef}>
            <Meta title={isEditMode ? "Edit Pre-Active Course" : "Add Pre-Active Course"} />
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex items-center justify-between shadow-sm">
                <div>
                    <h1 className="text-xl font-bold text-slate-800">
                        {isEditMode ? 'Edit Pre-Active Course' : 'Create Pre-Active Course'}
                    </h1>
                    <p className="text-sm text-slate-500">
                        {isEditMode ? `Updating ${formData.course_name}` : 'Setup a new pre-active course for nominations'}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <SelectField
                                            ...
                                            options={[
                                                ...
                                                { label: 'CBT', value: 'CBT' },
                                                { label: 'Inhouse (third party)', value: 'Inhouse (third party)' }
                                            ]}
                    ...
                    <div className="sticky bottom-0 bg-white border-t border-slate-200 p-4 sm:p-6 -mx-8 -mb-8 flex justify-end z-10 mt-8 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] rounded-b-xl">
                        <div className="flex gap-4 w-full sm:w-auto">
                            ...
                            <Button type="submit" disabled={loading} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-8 py-2.5 rounded-xl font-semibold shadow-lg shadow-blue-500/25 transition-all active:scale-95 disabled:opacity-70 text-sm min-w-[160px]">
                                {loading ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        <span>{isEditMode ? 'Update' : 'Create'} Pre-Active Course</span>
                                    </>
                                )}
                            </Button>
                        </div>
