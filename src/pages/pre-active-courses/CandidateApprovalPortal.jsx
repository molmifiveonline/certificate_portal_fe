...
const CandidateApprovalPortal = () => {
    ...
    if (success) {
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
                <div className={`w-full max-w-md bg-white rounded-xl shadow-lg p-8 text-center border ${isApproved ? 'border-green-100' : 'border-red-100'}`}>
                    <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full mb-6 ${isApproved ? 'bg-green-100' : 'bg-red-100'}`}>
                        {isApproved ? (
                    ...
                    <p className="text-gray-600 mb-6">
                        You have <strong>{status.toLowerCase()}</strong> the nomination for <strong>{courseContext?.course?.course_name}</strong>.
                    </p>
                    {isApproved && (
                        <div className="bg-slate-50 p-4 rounded-md text-sm text-gray-700 mb-6 text-left border border-slate-200">
                            <p className="font-semibold mb-2">Next Steps:</p>
                            <p>Your approval has been sent to the administrator for final confirmation. You will be notified once the course is confirmed.</p>
                        </div>
                    )}
            ...
            <div className="mx-auto max-w-2xl">
                ...
                <div className="bg-white shadow rounded-lg overflow-hidden mb-8 border border-slate-200">
                    <div className="bg-[#3a5f9e] px-6 py-4">
                        <h1 className="text-xl font-bold text-white">Course Nomination Review</h1>
                    </div>
                    <div className="p-6 md:p-8">
                        <div className="mb-6 pb-6 border-b border-gray-100">
                            ...
                            <p className="text-gray-600 leading-relaxed">
                                You have been nominated to participate in the following course. Please review the details below and accept or reject the nomination.
                            </p>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-lg p-5 mb-8 border border-slate-200">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="font-medium text-gray-900 text-lg">{courseContext?.course?.course_name}</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <p className="text-xs text-gray-500 mb-1">Start Date</p>
                                        <p className="font-medium text-gray-900">{new Date(courseContext?.course?.start_date).toLocaleDateString()}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 mb-1">End Date</p>
                                        <p className="font-medium text-gray-900">{new Date(courseContext?.course?.end_date).toLocaleDateString()}</p>
                                    </div>
                                </div>
                                <div>
