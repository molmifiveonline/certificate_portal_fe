...
import { Save, ArrowLeft, BookOpen, MapPin, Calendar, FileText, Video, Clock, Users, Link as LinkIcon, Check, ChevronDown, Trash2, Mail, AlertTriangle, AlertCircle, RefreshCcw } from 'lucide-react';
...
// ==========================================
const AttendanceRemarkModal = ({ isOpen, onClose, onSubmit, status, candidateName, date }) => {
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">{candidateName} - {formatDateDMY(date)}</p>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400"><Trash2 size={20} className="rotate-45" /></button>
                </div>
                <div className="p-6 space-y-4">
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-700">Reason / Remark <span className="text-red-500">*</span></label>
                        <textarea
                            autoFocus
                            value={reason}
                ...
                <div className="p-6 bg-slate-50 flex justify-end gap-3">
                    ...
                    <Button disabled={!reason.trim()} onClick={() => onSubmit(reason)}>Save Status</Button>
                </div>
            </div>
        </div>
    );
...
const FeedbackTab = ({ courseId }) => {
    ...
    if (loading) return <div className="flex justify-center py-12 text-slate-500">Loading...</div>;
                ...
                <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-left">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-semibold text-slate-600">Candidate Name</th>
                            <th className="px-6 py-3 font-semibold text-slate-600 text-center">Feedback Completed</th>
                        </tr>
                    </thead>
                    <tbody>
                        {candidates.length === 0 ? (
                            <tr><td colSpan="4" className="text-center py-8 text-slate-400">No candidates enrolled</td></tr>
                        ) : candidates.map((c, i) => (
                            <tr key={c.candidate_id} className="border-b border-slate-50 hover:bg-slate-25">
                                <td className="px-6 py-3 text-slate-500">{i + 1}</td>
                                <td className="px-6 py-3 font-mono text-slate-600">{c.empId || '-'}</td>
                                <td className="px-6 py-3 font-medium text-slate-800">{c.candidate_name}</td>
                                <td className="px-6 py-3 text-center">
                                    <input type="checkbox" checked={!!c.feedback_completed} readOnly className="h-4 w-4 accent-blue-600" />
                                </td>
                            </tr>
                        ))}
                    </tbody>
...
const AssessmentTab = ({ courseId }) => {
    ...
    const handleSendEmail = async (candidate) => {
        const assessmentId = candidate.post_assessment_id || candidate.pre_assessment_id;
        if (!assessmentId) return;
        setSendingEmail(candidate.candidate_id);
        try {
            await activeCourseService.sendAssessmentEmail(courseId, candidate.candidate_id, assessmentId);
                ...
                <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-left">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-semibold text-slate-600">Employee ID</th>
                            <th className="px-6 py-3 font-semibold text-slate-600">Candidate Name</th>
                            <th className="px-6 py-3 font-semibold text-slate-600 text-center">Pre Score</th>
                            <th className="px-6 py-3 font-semibold text-slate-600 text-center">Post Score</th>
                            <th className="px-6 py-3 font-semibold text-slate-600 text-center">Status</th>
                            <th className="px-6 py-3 font-semibold text-slate-600 text-center">Email</th>
                        </tr>
                    </thead>
                    <tbody>
                        {candidates.length === 0 ? (
                            <tr><td colSpan="7" className="text-center py-8 text-slate-400">No candidates enrolled</td></tr>
                        ) : candidates.map((c, i) => {
                            const hasAssessment = c.pre_assessment_id || c.post_assessment_id;
                            return (
                                <tr key={c.candidate_id} className="border-b border-slate-50 hover:bg-slate-25">
                                    <td className="px-6 py-3 text-slate-500">{i + 1}</td>
                                    <td className="px-6 py-3 font-mono text-slate-600">{c.empId || '-'}</td>
                                    <td className="px-6 py-3 font-medium text-slate-800">{c.candidate_name}</td>
                                    <td className="px-6 py-3 text-center">
                                        {c.pre_score !== null ? `${c.pre_score}/${c.pre_total}` : <span className="text-slate-400">-</span>}
                                    </td>
                                    <td className="px-6 py-3 text-center">
                                        {c.post_score !== null ? `${c.post_score}/${c.post_total}` : <span className="text-slate-400">-</span>}
                                    </td>
                                    <td className="px-6 py-3 text-center">
                                        {c.post_score !== null ? (
                                    ...
                                    <td className="px-6 py-3 text-center">
                                        <button
                                            ...
                                            disabled={!hasAssessment || sendingEmail === c.candidate_id}
                                            className={cn(
                                            ...
                                            title={hasAssessment ? "Send assessment result email" : "No assessment results yet"}
                                        >
                                            <Mail size={14} />
                                            {sendingEmail === c.candidate_id ? 'Sending...' : 'Email'}
                                        </button>
                                    </td>
                                </tr>
                            );
...
const AttendanceTab = ({ courseId }) => {
    ...
    const [loading, setLoading] = useState(true);
    const [remarkModal, setRemarkModal] = useState({ open: false, candidate: null, date: null, status: null });
        ...
        const fetchData = async () => {
            try {
                ...
                if (res.start_date && res.end_date) {
                    setDates(generateDateRange(res.start_date.slice(0, 10), res.end_date.slice(0, 10)));
                }
            } catch (err) {
    ...
    const getReason = (candidate, date) => {
        ...
        try {
            const reasons = typeof candidate.absent_reasons === 'string'
    ...
    const handleToggle = async (candidate, date) => {
        ...
        const nextStatus = currentStatus === 'absent' ? 'present' : currentStatus === 'present' ? 'holiday' : 'absent';
    ...
    const saveAttendance = async (candidate, date, status, reason = null) => {
        try {
            await activeCourseService.saveAttendanceSingle(courseId, candidate.candidate_id, date, status, reason);
            // Update local state
            setCandidates(prev => prev.map(c => {
                if (c.candidate_id !== candidate.candidate_id) return c;
                let present = c.is_present ? c.is_present.split(',').filter(Boolean) : [];
                let holidays = c.holidays ? c.holidays.split(',').filter(Boolean) : [];
                let reasons = {};
                try {
                    reasons = c.absent_reasons ? (typeof c.absent_reasons === 'string' ? JSON.parse(c.absent_reasons) : c.absent_reasons) : {};
                } catch (e) { }
            ...
            if (remarkModal.open) {
                setRemarkModal({ open: false, candidate: null, date: null, status: null });
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100">
                ...
                <p className="text-sm text-slate-500 mt-1">Click cells to cycle: <span className="text-red-500 font-medium">Absent</span> → <span className="text-green-600 font-medium">Present</span> → <span className="text-yellow-600 font-medium">Holiday</span></p>
            </div>
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-left">
                        <tr>
                            <th className="px-4 py-3 font-semibold text-slate-600 sticky left-0 bg-slate-50 z-10 min-w-[200px]">Candidate</th>
                            {dates.map(date => (
                                <th key={date} className="px-2 py-3 font-semibold text-slate-600 text-center text-xs whitespace-nowrap">
                    ...
                    <tbody>
                        {candidates.length === 0 ? (
                            <tr><td colSpan={1 + dates.length} className="text-center py-8 text-slate-400">No candidates enrolled</td></tr>
                        ) : candidates.map(c => (
                            <tr key={c.candidate_id} className="border-b border-slate-50">
                                <td className="px-4 py-2 font-medium text-slate-800 sticky left-0 bg-white z-10">
                                    ...
                                    <div className="text-xs text-slate-400">{c.empId || ''}</div>
                                </td>
                                {dates.map(date => {
                                    const status = getStatus(c, date);
                                    const reason = getReason(c, date);
                                    const bg = status === 'present' ? 'bg-green-100 text-green-700 hover:bg-green-200'
                                        : status === 'holiday' ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'
                                            : 'bg-red-50 text-red-500 hover:bg-red-100';
                                    const label = status === 'present' ? 'P' : status === 'holiday' ? 'H' : 'A';
                                    return (
                                        <td key={date} className="px-1 py-2 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                {reason && (
                                                    <span className="text-[10px] text-slate-400 mt-0.5 max-w-[40px] truncate" title={reason}>
                                                        {reason}
                                                    </span>
                                                )}
                                            </div>
            ...
            <AttendanceRemarkModal
                ...
                onClose={() => setRemarkModal({ open: false, candidate: null, date: null, status: null })}
                status={remarkModal.status}
                candidateName={remarkModal.candidate?.candidate_name}
                date={remarkModal.date}
                onSubmit={(reason) => saveAttendance(remarkModal.candidate, remarkModal.date, remarkModal.status, reason)}
            />
        </div>
    );
};
...
const CertificateTab = ({ courseId }) => {
    ...
    const calcAttendance = (candidate) => {
        ...
        const present = candidate.is_present ? candidate.is_present.split(',').filter(Boolean) : [];
        const holidays = candidate.holidays ? candidate.holidays.split(',').filter(Boolean) : [];
    ...
    const handleToggleActive = async (candidateId, currentValue) => {
        try {
            await activeCourseService.updateCertificateActive(courseId, candidateId, currentValue ? 0 : 1);
            setCandidates(prev => prev.map(c =>
                c.candidate_id === candidateId ? { ...c, active: currentValue ? 0 : 1 } : c
            ));
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100">
                ...
                <p className="text-sm text-slate-500 mt-1">Eligible: 100% attendance + ≥60 post score + feedback completed</p>
            </div>
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-left">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-semibold text-slate-600 text-center">Active</th>
                            <th className="px-4 py-3 font-semibold text-slate-600 text-center">Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        {candidates.length === 0 ? (
                            <tr><td colSpan="9" className="text-center py-8 text-slate-400">No candidates enrolled</td></tr>
                        ) : candidates.map((c, i) => {
                            ...
                            return (
                                <tr key={c.candidate_id} className="border-b border-slate-50 hover:bg-slate-25">
                                    ...
                                    <td className="px-4 py-3 font-mono text-slate-600">{c.empId || '-'}</td>
                                    <td className="px-4 py-3 font-medium text-slate-800">{c.candidate_name}</td>
                                    <td className="px-4 py-3 text-center">
                                        <span className={c.post_score >= 60 ? 'text-green-600 font-semibold' : 'text-red-500 font-semibold'}>
                                            {c.post_score ?? '-'}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 text-center">
                                        <span className={attendance >= 100 ? 'text-green-600 font-semibold' : 'text-red-500 font-semibold'}>
                                            {attendance}%
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 text-center">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        {c.certficate_generated ? (
                                            <Badge className="bg-green-100 text-green-700 border-green-200 cursor-default">Generated</Badge>
                                        ) : (
                                            <Button
                                                size="sm"
...
const ActiveCourseForm = () => {
    ...
    const [emailModal, setEmailModal] = useState({ isOpen: false, candidateId: null, type: null }); // type: 'online' | 'offline'

    const [venueModal, setVenueModal] = useState({ isOpen: false, candidateId: null, data: null });
    const [deleteModal, setDeleteModal] = useState({ isOpen: false, candidateId: null, remark: '' });
            ...
            const fetchData = async () => {
                try {
                    ...
                    reset({
                        ...
                        secondary_trainer_ids: course.secondary_trainer_ids ? course.secondary_trainer_ids.split(',') : [],
                    ...
                    // Fetch Candidates
                    const candidates = await activeCourseService.getEnrolledCandidates(id);
    ...
    useEffect(() => {
        if (selectedTopic && masterCourses.length > 0) {
            ...
            if (selectedMC) {
                ...
                if (!id) { // Only overwrite description/remarks on create
    ...
    const handleAddCandidates = async () => {
        try {
            await activeCourseService.enrollCandidates(id, selectedCandidates, courseData.primary_trainer_id);
    ...
    const handleDeleteCandidate = async () => {
        ...
        try {
            await activeCourseService.removeCandidate(id, deleteModal.candidateId, deleteModal.remark);
            setEnrolledCandidates(prev => prev.filter(c => c.candidate_id !== deleteModal.candidateId));
    ...
    const handleStatusPoolChange = async (candidateId, newStatus) => {
        try {
            ...
            setEnrolledCandidates(prev => prev.map(c =>
    ...
    const handleEmailPrimaryTrainer = async () => {
        if (!window.confirm("Send enrollment confirmation email to Primary Trainer?")) return;
        try {
    ...
    const handleSaveVenue = async (e) => {
        ...
        try {
            await activeCourseService.updateCandidateVenue(id, venueModal.candidateId, formData);
    ...
    // Helper Components
    const InputField = ({ label, name, type = "text", required, readOnly, placeholder }) => (
    ...
    const SelectField = ({ label, name, options, required, disabled, children }) => (
        <div className="space-y-1">
            <label className="text-sm font-medium text-slate-700 block">
                {label} {required && <span className="text-red-500">*</span>}
            </label>
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        {courseData && <p className="text-sm text-slate-500">{courseData.course_name} ({courseData.course_id})</p>}
                    </div>
                </div>
                {id && (
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={handleEmailPrimaryTrainer} className="gap-2">
                            <Mail size={16} /> Email Primary Trainer
                        </Button>
                        {(!['Cancelled', 'Course Completed'].includes(courseData?.status)) && (
                            <>
                                <Button variant="destructive" onClick={() => setActionModal({ isOpen: true, type: 'cancel', reason: '' })}>
                                    Cancel Course
                                </Button>
                                <Button className="bg-green-600 hover:bg-green-700" onClick={() => setActionModal({ isOpen: true, type: 'complete', reason: '' })}>
                                    Complete Course
                                </Button>
                            </>
                        )}
            ...
            <div className="max-w-none p-8 space-y-8">
                ...
                {id && (
                    <div className="flex border-b border-slate-200">
                        {['details', 'attendance', 'assessment', 'feedbacks', 'certificates'].map(tab => (
                            <button
                                key={tab}
                ...
                {activeTab === 'details' && (
                    <div className="space-y-8">
                        <form id="course-form" onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className="space-y-8">
                                        ...
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-2 gap-4">
                                                <SelectField
                                                    ...
                                                    name="topic"
                                                    required
                                                    options={masterCourses.map(mc => ({ value: mc.id, label: mc.topic }))}
                                                />
                                                <InputField label="Course Name" name="course_name" required />
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <SelectField
                                                    ...
                                                    name="status"
                                                    disabled
                                                    options={['Initiated', 'Active', 'Cancelled', 'Course Completed'].map(s => ({ value: s, label: s }))}
                                                />
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <InputField label="Course Level" name="course_level" />
                                                <InputField label="Type of Course" name="course_type" />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <InputField label="No. of Days" name="no_of_days" type="number" />
                                                <SelectField
                                                    label="Location Type"
                                            ...
                                            {typeOfLocation === 'Offline' && (
                                                <SelectField label="Venue / Location" name="location_id">
                                                    {locations.map(loc => (
                                            ...
                                            {((typeOfLocation === 'Offline' && watch('location_id') === 'Other') || typeOfLocation === 'Manual') && (
                                            ...
                                            {typeOfLocation === 'Online' && (
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <InputField label="WhatsApp Link" name="whatsapp_link" />
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                        ...
                                        <div className="space-y-6">
                                            <SelectField
                                                ...
                                                required
                                                options={trainers.map(t => ({ value: t.id, label: `${t.first_name} ${t.last_name}` }))}
                                            />
                                            <div className="space-y-1">
                                                <label className="text-sm font-medium text-slate-700 block">Secondary Trainers</label>
                                                <Controller
                                                    control={control}
                            ...
                            {!id && (
                                <div className="sticky bottom-0 bg-white border-t border-slate-200 p-4 sm:p-6 -mx-8 -mb-8 flex justify-end z-10 mt-8 rounded-b-xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                                            ...
                                            {enrolledCandidates.length === 0 ? (
                                                ...
                                                enrolledCandidates.map(candidate => (
                                                    <tr key={candidate.id} className="hover:bg-slate-50">
                                                        <td className="px-4 py-3 text-sm text-slate-600">{candidate.empId}</td>
                                                        <td className="px-4 py-3 text-sm font-medium text-slate-900">{candidate.candidate_name}</td>
                                                        <td className="px-4 py-3 text-sm text-slate-600">{candidate.rank}</td>
                                                        <td className="px-4 py-3 text-sm text-slate-600">{candidate.cdc_passport}</td>
                                                        <td className="px-4 py-3 text-sm text-slate-600">{candidate.seaman_book_no}</td>
                                                        <td className="px-4 py-3 text-sm text-slate-600">{candidate.manager}</td>
                                                        <td className="px-4 py-3">
                                                            <select
                                                                value={candidate.status_pool || ''}
                                                                onChange={(e) => handleStatusPoolChange(candidate.candidate_id, e.target.value)}
                                                        ...
                                                        <td className="px-4 py-3 text-right flex items-center justify-end gap-2">
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => setEmailModal({ isOpen: true, candidateId: candidate.candidate_id })}
                                                                title="Send Email"
                                                            >
                                                                <Mail size={16} className={candidate.candidate_email_status ? "text-green-500" : "text-slate-400"} />
                                                            </Button>
                                                            {typeOfLocation !== 'Online' && (
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
                                                                    onClick={() => openVenueModal(candidate.candidate_id)}
                                                                    title="Edit Venue"
                                                                >
                                                                    {/* Use a pencil icon */}
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => setDeleteModal({ isOpen: true, candidateId: candidate.candidate_id, remark: '' })}
                                                            >
                                                                <Trash2 size={16} />
                                                            </Button>
                                                        </td>
                ...
                {activeTab === 'assessment' && (
                    <AssessmentTab courseId={id} />
                )}

                {/* Feedbacks Tab */}
                {activeTab === 'feedbacks' && (
                    <FeedbackTab courseId={id} />
                )}

                {/* Certificates Tab */}
                {activeTab === 'certificates' && (
                    <CertificateTab courseId={id} />
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-red-50">
                            ...
                            <button onClick={() => setDeleteModal({ isOpen: false, candidateId: null, remark: '' })} className="text-slate-400 hover:text-slate-600">×</button>
                        </div>
                        <div className="p-6 space-y-4">
                            <p className="text-slate-600 text-sm">
                                Are you sure you want to remove this candidate? This action will mark them as deleted.
                                Please provide a reason.
                            </p>
                            <div className="space-y-1">
                                <label className="text-sm font-medium text-slate-700">Reason / Remark</label>
                                <textarea
                                    ...
                                    onChange={(e) => setDeleteModal({ ...deleteModal, remark: e.target.value })}
                                />
                            </div>
                            <div className="flex justify-end gap-3 pt-2">
                                <Button variant="ghost" onClick={() => setDeleteModal({ isOpen: false, candidateId: null, remark: '' })}>Cancel</Button>
                                <Button variant="destructive" onClick={handleDeleteCandidate} disabled={!deleteModal.remark}>Remove Candidate</Button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-4xl max-h-[80vh] flex flex-col shadow-2xl">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsCandidateModalOpen(false)} className="text-slate-400 hover:text-slate-600">×</button>
                        </div>
                                ...
                                <tbody>
                                    {availableCandidates
                                        .filter(c => {
                                            ...
                                            return name.includes(search) || c.empId?.toLowerCase().includes(search);
                                        })
                                        .map(candidate => (
                                            <tr key={candidate.id} className="hover:bg-slate-50 border-b border-slate-50">
                                                <td className="px-4 py-3">
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedCandidates.includes(candidate.id)}
                                                        onChange={(e) => {
                                                            if (e.target.checked) setSelectedCandidates([...selectedCandidates, candidate.id]);
                                                            else setSelectedCandidates(selectedCandidates.filter(id => id !== candidate.id));
                                                        }}
                                                        className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                                    />
                                                </td>
                                                <td className="px-4 py-3 font-medium">{candidate.first_name} {candidate.last_name}</td>
                                                <td className="px-4 py-3 text-slate-500">{candidate.empId}</td>
                        ...
                        <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 rounded-b-xl">
                            ...
                            <Button onClick={handleAddCandidates} disabled={selectedCandidates.length === 0}>
            ...
            {actionModal.isOpen && (
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl p-6">
                        <h3 className="text-lg font-bold mb-4 capitalize">{actionModal.type} Course</h3>
                        <p className="text-sm text-slate-500 mb-4">
                            Please provide a reason for {actionModal.type === 'cancel' ? 'cancelling' : 'completing'} this course.
                        </p>
                        <textarea
                        ...
                        <div className="flex justify-end gap-3">
                            <Button variant="outline" onClick={() => setActionModal({ isOpen: false, type: null, reason: '' })}>Cancel</Button>
                            <Button
                                variant={actionModal.type === 'cancel' ? 'destructive' : 'default'}
                                onClick={handleCourseAction}
                                className={actionModal.type === 'complete' ? 'bg-green-600 hover:bg-green-700' : ''}
                            >
                                Confirm {actionModal.type === 'cancel' ? 'Cancellation' : 'Completion'}
                            </Button>
                        </div>
                    </div>
                </div>
            ...
            {emailModal.isOpen && (
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl p-6">
                        ...
                        <p className="text-sm text-slate-500 mb-6">Select the type of email to send.</p>
                        <div className="flex flex-col gap-3">
                            <Button onClick={() => handleSendOnlineEmail(emailModal.candidateId)} className="w-full justify-start gap-2">
                                <Video size={16} /> Online Class (Zoom)
                            </Button>
                            <Button variant="outline" onClick={() => handleSendOfflineEmail(emailModal.candidateId)} className="w-full justify-start gap-2">
                                <MapPin size={16} /> Offline Class (Venue)
                            </Button>
                        </div>
                        <div className="mt-6 flex justify-end">
                            <Button variant="ghost" onClick={() => setEmailModal({ isOpen: false, candidateId: null })}>Cancel</Button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSaveVenue} className="space-y-4">
                            <InputField label="Venue Name" name="venue_name" defaultValue={venueModal.data?.venue_name} placeholder="e.g. Hotel ..." />
                            <InputField label="Venue Address" name="venue_address" defaultValue={venueModal.data?.venue_address} />
                            <div className="grid grid-cols-2 gap-4">
                                <InputField label="Contact" name="venue_contact" defaultValue={venueModal.data?.venue_contact} />
                                <InputField label="Email" name="venue_email" defaultValue={venueModal.data?.venue_email} />
                            </div>
                            <InputField label="Map Link" name="venue_map_link" defaultValue={venueModal.data?.venue_map_link} />

                            <div className="grid grid-cols-2 gap-4">
                                <InputField label="Offline Date" name="offline_date" type="date" defaultValue={venueModal.data?.offline_date} />
                                <InputField label="Remarks" name="remarks" defaultValue={venueModal.data?.remarks} />
                            </div>

                            <div className="space-y-1">
                                <label className="text-sm font-medium text-slate-700 block">Attach Files</label>
                                <input type="file" name="venue_files" multiple className="text-sm" />
                            ...
                            <div className="flex justify-end gap-3 pt-4">
                                <Button type="button" variant="outline" onClick={() => setVenueModal({ isOpen: false, candidateId: null, data: null })}>Cancel</Button>
