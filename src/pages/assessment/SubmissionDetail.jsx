...
import { ArrowLeft, User, Mail, Award, CheckCircle, Hash, ClipboardCheck, FileDown, XCircle } from "lucide-react";
                    ...
                    <div className="space-y-10">
                        {answers.length > 0 ? (
                            answers.map((ans, index) => {
                                ...
                                return (
                                    <div key={ans.id} className="pb-8 border-b border-slate-100 last:border-0 last:pb-0">
                                        {/* Question Header */}
                                        <div className="flex items-start gap-4 mb-4">
                                            <div className="flex-shrink-0 flex items-baseline">
                                                <span className="text-4xl font-light text-slate-800 tracking-tighter mr-1">Q</span>
                                                <span className="text-lg font-medium text-slate-700">{index + 1}.</span>
                                            </div>
                                        ...
                                        <div className="ml-14 space-y-1">
                                            {options.map((opt) => {
                                                ...
                                                const isSelected = ans.selected_option === opt.key;
                                                const isCorrect = ans.correct_option === opt.key;

                                                let rowStyle = "border-b-2 border-slate-100 bg-white text-slate-600";
                                                let iconHtml = null;
                                                let correctLabel = null;

                                                if (isCorrect && !isSelected) {
                                                    rowStyle = "border-b-2 border-slate-100 bg-white text-slate-600";
                                                    correctLabel = <span className="text-green-600 font-bold ml-2 text-sm">(Correct Answer)</span>;
                                                }
                                                if (isCorrect && isSelected) {
                                                    rowStyle = "border-b-2 border-green-500 bg-green-100/50 text-slate-800";
                                                    iconHtml = <i className="fas fa-check text-green-600 ml-3"></i>;
                                                }
                                                if (isSelected && !isCorrect) {
                                                    rowStyle = "border-b-2 border-slate-100 bg-white text-slate-600";
                                                    iconHtml = <i className="fas fa-times text-red-500 ml-3"></i>;
                                                }

                                                return (
                                                    <div key={opt.key} className={`flex items-start flex-col px-3 py-3 transition-colors ${rowStyle}`}>
                                                        {opt.img && (
                                                            <img
                                                                src={opt.img}
