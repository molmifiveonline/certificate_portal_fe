...
const SystemManualCategoryForm = ({ isOpen, onClose, onSuccess, initialData }) => {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            if (initialData?.id) {
                setLoading(true);
                try {
                    const response = await systemManualCategoryService.getCategoryById(initialData.id);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (initialData) {
                await systemManualCategoryService.updateCategory(initialData.id, formData);
                toast.success("System Manual Category updated successfully");
            } else {
                await systemManualCategoryService.createCategory(formData);
                toast.success("System Manual Category created successfully");
            }
            onSuccess();
            onClose();
        } catch (error) {
            console.error("Error saving system manual category:", error);
                ...
                <form onSubmit={handleSubmit} noValidate className="p-6 space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-4 py-2 bg-white/50 border ${formErrors.name ? 'border-red-500' : 'border-slate-200'} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm`}
...
export default SystemManualCategoryForm;
