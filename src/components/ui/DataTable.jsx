...
const DataTable = ({
  ...
  return (
    <div className="bg-white/60 backdrop-blur-2xl rounded-3xl border border-white/40 shadow-xl overflow-hidden flex flex-col relative">
      {/* Overlay loader when paginating/refreshing data */}
      {loading && data.length > 0 && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/20 backdrop-blur-[1px]">
          <div className="flex flex-col items-center gap-2 bg-white/90 p-4 rounded-xl shadow-sm border border-slate-100">
      ...
      <div className="overflow-x-auto w-full">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gradient-to-r from-indigo-900/90 via-blue-900/90 to-indigo-900/90 text-white border-b border-white/10 backdrop-blur-md">
              ...
              {columns.map((col) => {
                ...
                const responsiveCls = col.hiddenOnMobile
                ...
                if (col.sortable && onSort) {
                  return (
                    <th
                      key={col.key}
                      className={cn(
                        "px-6 py-4 text-xs font-bold text-white uppercase tracking-wider cursor-pointer hover:text-white/80 transition-colors select-none",
                        alignCls,
                        responsiveCls,
                      )}
                ...
                return (
                  <th
                    key={col.key}
                    className={cn(
                      "px-6 py-4 text-xs font-bold text-white uppercase tracking-wider",
                      alignCls,
                      responsiveCls,
                    )}
                  >
          ...
          <tbody
            className={cn(
              "divide-y divide-slate-100/50 transition-opacity duration-200",
              loading && data.length > 0
